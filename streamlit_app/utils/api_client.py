...
def transform_profile_for_recommendation_api(user_profile):
    ...
    def extract_title(book_entry):
        if isinstance(book_entry, dict):
            return f"{book_entry.get('title', '').strip()} - {book_entry.get('author', '').strip()}"
        elif isinstance(book_entry, str):
            return book_entry.strip()
        return ""
                ...
                if response.status_code == 200:
                    ...
                    for rec in raw_recommendations:
                        ...
                        try:
                            ...
                            # Update book metadata cache with information from this recommendation
                            update_book_metadata(processed_rec["title"], {
                                "title": processed_rec["title"],
                                "author": processed_rec["author"],
                                "genre": rec_data.get("genre", "Unknown Genre")  # Extract genre if available
                            })
